...
8. Chequear que exista el acceso directo a `/storage`. Sino crearlo con: `php artisan storage:link`
9. Correr migraciones: `php artisan migrate`.
10. Correr seeder: `php artisan db:seed`.
...
3. Configurar archivo `.env` con los datos de la DB creada.
4. Crear link a storage: `php artisan storage:link`
5. Correr Auth: `php artisan make:auth`
6. Instalar Doctrine (por si fuera necesario hacer `AlterTable` en alguna tabla de la db).
7. Teniendo en cuenta las tablas de nuestra db, crear los modelos con el comando `-a` para que creen todos los recursos: *Model, Migration, Factory y Controller*
8. Si Laravel devuelve error sql:
